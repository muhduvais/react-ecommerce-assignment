...
const CATEGORIES = [
  "all", "smartphones", "laptops", "fragrances", "beauty",
  "groceries", "home-decoration", "furniture", "tops",
  "womens-dresses", "mens-shirts",
];
...
function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-8 py-8">
      <div className="mb-8">
        <h1 className="font-serif text-3xl font-medium text-stone-900 mb-1 capitalize">
      ...
      <div className="flex flex-wrap gap-4 items-start mb-8">
        <div className="flex flex-wrap gap-2 flex-1">
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`text-[12px] px-4 py-1.5 rounded-full border transition-colors capitalize ${
                category === cat
        ...
        <select
          ...
          className="text-[13px] px-3 py-1.5 rounded-lg border border-[#E5E1DA] bg-white text-stone-600 min-w-40"
        >
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mb-8">
          {Array.from({ length: PAGE_SIZE }).map((_, i) => (
            <div key={i} className="bg-white rounded-xl border border-[#EEEBE6] h-64 animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 mb-8">
